...
# Initialize the layers of Forward Neural Network
...
# Implements tanh, sigmoid and relu
def logistic(val):
  temp = []
...
# Main function called in the config file
def trainD(layer_inputX, wt_initialisationX, activation_functionX, epochsX, etaX, optimizerX, betaX, epsilonX, beta1X, beta2X, batch_sizeX):
  ...
  for test in range (54000,60000):
    ...
    if (maxI == trainY[test]):
      ...
      val_loss += loss_function(res[trainY[test]])


  # Test Data
  for test in range (0,54000):
      x = trainX[test]
      x = x.ravel()
      x = x.reshape(784,1)
      res = fwd_pro(x)
  ...
  # Plots graphs on wandb
  wandb.log({'train_loss':loss/54000,'train_accuracy':accuracy/54000,'val_accuracy':val_accuracy/6000,'val_loss':val_loss/6000})
...
sweep_id = wandb.sweep(sweep_config,project="DLA1", entity="uditchaurasia")
